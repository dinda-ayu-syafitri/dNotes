...
function ArchivedNotes() {
  ...
  const fetchNotesData = async (userID) => {
    try {
      ...
      if (error) {
        ...
        setNotes(data.filter((note) => note.status === "archived"));
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div>
      <PageHeading title="Archived Notes" />
      {notes.length > 0 ? (
        ...
        <div className="text-center flex justify-center items-center h-screen -my-10">
